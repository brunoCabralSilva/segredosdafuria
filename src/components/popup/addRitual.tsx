...
export default function AddRitual() {
  ...
  const selectedRituals = Array.isArray(dataSheet?.data?.rituals) ? dataSheet.data.rituals : [];
          ...
          <div className="mt-4 space-y-2.5 pb-10">
            {selectedRituals.length === 0 ? (
              ...
              selectedRituals.map((item: any, index: number) => (
                <div key={`${item.title}-${index}`} className="border border-red-700/60 bg-black/55 px-4 py-3 shadow-[0_0_20px_rgba(127,29,29,0.16)]">
