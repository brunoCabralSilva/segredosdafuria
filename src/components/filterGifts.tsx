...
export default function FilterGifts(props: { type: string, title: string }) {
  ...
  return (
    <div className="">
      <div
        className="font-bold my-1 py-2 px-5 text-base flex justify-between items-center cursor-pointer bg-black"
      >
        <p className="w-full text-center sm:text-left">{ title }</p>
      </div>
      { 
        ...
        <div className="grid grid-cols-1 mobile:grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-1 mb-2 w-full">
          {
            list.length > 0 && list.map((item: string | number, index: number) => (
              <motion.div
                ...
                key={ index }
                className={`w-full flex items-center justify-center px-2 cursor-pointer border-2 ${selectTA.includes(item) ? 'border-white bg-black' : 'border-transparent bg-black'}`}
