...
import { reduceFdv, regenerateFdv, verifyRage } from "../functionGifts";
...
import { generateDataRoll } from "@/app/sessions/[sessionId]/functions";

export default function DrainSpirit(props: any) {
  ...
  const rollDice = async () => {
    ...
    if (rage) {
      ...
      const dtSheet: any | null = await returnValue('resolve', '', 'glory', slice.showPopupGiftRoll.gift.session);
      if (dtSheet) {
        ...
        try {
          if (authData && authData.email && authData.name) {
            const generate = generateDataRoll({ rollOfRage: resultOfRage, rollOfMargin: resultOf, dificulty: dificulty })
            if (generate.success < dificulty ){
              await reduceFdv(slice.showPopupGiftRoll.gift.session, true);
            } else {
              await regenerateFdv(slice.showPopupGiftRoll.gift.session, generate.success + 1 - dificulty);
            }
  ...
  return(
    <div className="w-full">
      ...
      <label htmlFor="dificulty" className="px-4 mb-4 flex flex-col items-center w-full">
        <p className="text-white w-full pb-3 text-justify">Dificuldade (A dificuldade deve ser o número de sucessos obtidos pelo espírito alvo em um teste de Poder, ou um valor imposto pelo Narrador).</p>
        <div className="flex w-full">
          <div
