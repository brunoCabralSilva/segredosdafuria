...
export default function Staredown() {
  ...
  const rollDice = async () => {
    if (reflex) {
      ...
      if (rage) {
        ...
        const dtSheet: any | null = await returnValue('charisma', '', 'honor', slice.showPopupGiftRoll.gift.session);
        if (dtSheet) {
  ...
  return(
    <div className="w-full">
      <label
        ...
        />
        <span>Marque se a ação for contra criaturas sobrenaturais ou humanos cientes dos Garou e seus poderes</span>
      </label>
      { reflex &&
        <div className="w-full">
