  ...
  const updateValue = async (newForm: string) => {
    ...
    if (session.typeSession == 'Regras Alternativas') {
      ...
      if (dataSheet.data.rage >= 5) {
        setShowMessage({show: true, text: `O personagem ${dataSheet.data.name} possui Fúria 5, se encontra em Frenesi e não pode realizar nenhuma ação que não seja atacar ou sair do mesmo (caso seja possível no momento).`});
