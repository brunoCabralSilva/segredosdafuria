...
const getTalismanPreview = (text: string, totalLength = 170) => {
...
export default function ListTalismans() {
  ...
  const totalPages = Math.max(1, Math.ceil(listOfTalismans.length / ITEMS_PER_PAGE));
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const paginatedTalismans = listOfTalismans.slice(startIndex, endIndex);
    ...
    <section className="space-y-4">
      <div className="border border-zinc-500/30 bg-black/80 p-5 text-white">
        <p className="font-geist-mono text-[11px] uppercase tracking-[0.12em] text-white/72">
          Total de Talismãs Encontrados: <span className="text-white">{listOfTalismans.length}</span>
        </p>
        {listOfTalismans.length > 0 && (
          <p className="mt-3 font-geist-mono text-[11px] uppercase tracking-[0.12em] text-white/55">
            Mostrando {startIndex + 1}-{Math.min(endIndex, listOfTalismans.length)}
          </p>
        )}
      </div>
      ...
      {listOfTalismans.length > 0 ? (
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {paginatedTalismans.map((item: ITalisman, index: number) => {
              const cardPreview = getTalismanPreview(String(item.descriptionPtBr || item.description || ''));
        ...
        <div className="border border-zinc-500/30 bg-black/80 p-5 text-white">
          <p className="font-geist-mono text-[11px] leading-6 text-white/70 sm:text-xs">
            Nenhum talismã encontrado com os filtros atuais.
          </p>
        </div>
      )}
    </section>
