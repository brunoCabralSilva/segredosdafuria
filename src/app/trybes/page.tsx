...
const mainTrybes = sortedTrybes.filter((trybe) => !trybe.custom);
const alternativeTrybes = sortedTrybes.filter((trybe) => trybe.custom);
...
function TrybeCard({ trybe }: { trybe: (typeof listTrybes)[number] }) {
          ...
          <section className="relative overflow-hidden text-white">
            <div className="relative px-5 pb-8 sm:px-8">
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {mainTrybes.map((trybe) => (
                  <TrybeCard key={trybe.nameEn} trybe={trybe} />
                ))}
              </div>

              <div className="mt-10">
                <h2 className="font-kingthings text-2xl text-white sm:text-3xl lg:text-4xl">Tribos alternativas</h2>
                <hr className="mt-4" />

                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {alternativeTrybes.map((trybe) => (
                    <TrybeCard key={trybe.nameEn} trybe={trybe} />
                  ))}
                </div>
              </div>
