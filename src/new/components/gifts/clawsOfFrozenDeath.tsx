...
export function ClawsOfFrozenDeath() {
  ...
  const rollRage = async () => {
    if (dataSheet.rage >= 1) {
      let roll = null;
      await rollTestOfUser();
      if (dataSheet.forms !== 'Crinos') {
      ...
      await registerMessage(sessionId, { type: 'gift', ...showGiftRoll.gift, roll: 'rage-with-test', results: roll }, email);
      returnSheetValues();
  ...
  return(
    <div className="w-full">
      <label
        ...
        />
        <span>Dificuldade (A dificuldade deve ser o número de sucessos obtidos pelo alvo em um teste de Vigor + Sobrevivência, ou um valor imposto pelo Narrador).</span>
      </label>
      {
        marked &&
