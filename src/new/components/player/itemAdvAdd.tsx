...
import DelAdvantageOrFlaw from "../popup/delAdvantageOrFlaw";
...
export default function ItensAdvantagesAdded(props: any) {
  ...
  const getAllAdvantages = async () => {
    const playerFound = await getPlayerByEmail(sessionId, email);
  ...
  return(
    <div>
      ...
      { deleteAdvOrFlaw.show && <DelAdvantageOrFlaw adv={adv} setAdv={setAdv} /> }
    </div>
  );
}