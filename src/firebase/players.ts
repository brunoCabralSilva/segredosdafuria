...
export const updateDataPlayer = async (
  ...
  try {
    const db = getFirestore(firebaseConfig);
    const docRef = doc(db, 'players', id);
    const normalizedData = normalizePlayerSheetForStorage(newData);
...
export const addNewSheet = async (sessionId: string, sheet: any, setShowMessage: any) => {
  try {
    ...
    const newDoc = normalizePlayerSheetForStorage({ sessionId: sessionId, ...sheet });
...
export const addNewSheetMandatory = async (sessionId: string, sheet: any, setShowMessage: any) => {
  try {
    ...
    const newDocRef = await addDoc(sessionsCollectionRef, normalizePlayerSheetForStorage({ sessionId, ...sheet }));
    return newDocRef.id;
  } catch (error: any) {
